...
select max(period_diff(date_format(curdate(),'%Y%m'),date_format(hire_date,'%Y%m')))
from employees;
...
select max(salary)as '최고연봉', min(salary) as '최저 연봉'
from salaries;

-- 문제6.
-- 최고 어린 사원의 나이와 최 연장자의 나이는?

select max(period_diff(date_format(curdate(),'%Y%m'),date_format(birth_date,'%Y%m')))/12 as '최연장자',min(period_diff(date_format(curdate(),'%Y%m'),date_format(birth_date,'%Y%m')))/12 as '최연소자'	
from employees;
